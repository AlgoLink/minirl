...
class Registry(object):
    ...
    def register(self, name, prim):
        ...
        if prim.type in self._reg[name]:
            _logger.warning(
                "Type %s for name %s has already existed", prim.typestr, name
            )
        else:
            _logger.debug('Function "%s" registered with type %s', name, prim.typestr)
        ...
        if name not in self._reg:
            ...
            for prim in self._reg[name].values():
                if prim.gradable(bp_args, bp_kwargs):
                    ...
                    _logger.info(
                        "The %s implementation: %s has no gradient " "definition.",
                        prim.typestr,
                        prim,
                    )
            return ret
