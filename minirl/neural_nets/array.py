...
class Value(object):
    ...
    def is_marked_for_bp(self, tape):
        ...
        return (
            tape != None and tape.is_recording and self._bp_timestamp == tape.timestamp
        )
...
class Array(Value):
    ...
    def _synchronize_data(self):
        ...
        if self._latest_version == ArrayType.MXNET:
            _logger.info(
                'Copy from MXNet array to NumPy array for Array "{}" of shape {}.'.format(
                    id(self), self.shape
                )
            )
            ...
            _logger.info(
                'Copy from NumPy array to MXNet array for Array "{}" of shape {}.'.format(
                    id(self), self.shape
                )
            )
