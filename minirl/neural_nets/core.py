    ...
    def _create_prim(self, global_dict):
        ...
        def func(*args):
            ...
            return (
                ...
                if len(executor.outputs) > 1
                else executor.outputs[0]
            )
        ...
        def grad_wrapper(ans, *args):  # pylint: disable= unused-argument
            def grad_func(g):
                ...
                for aux, executor_aux in zip(
                    global_dict["aux_states"], executor.aux_arrays
                ):
                    if aux is not None:
                        aux[:] = executor_aux
        ...
        prim.def_multiple_grad(
            grad_wrapper, tuple(range(len(arg_names) + len(aux_state_names)))
        )
        return prim
        # pylint: enable= missing-docstring
