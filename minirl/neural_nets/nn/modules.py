...
class Symbolic(minpy.nn.model_builder.Layer):
    ...
    def __init__(
        self, symbol, variables=None, init_configs=None, update_configs=None, name=None
    ):
        self._symbol = symbol
    ...
    def forward(self, **kwargs):
        ...
        kwargs.update(
            dict(zip(self._module_param_names, self._get_params(*self._param_names)))
        )
        kwargs.update(
            dict(
                zip(
...
class RNN(Symbolic):
    ...
    def __init__(self, num_hidden, act_type):
        ...
        data = mxnet.symbol.FullyConnected(
            data=data, num_hidden=num_hidden, no_bias=True
        )
        hidden = mxnet.symbol.FullyConnected(
            data=hidden, num_hidden=num_hidden, name="HH"
        )
        network = data + hidden
        ...
        self._register_init_configs(
            {self.HH_weight: {"value": minpy.numpy.eye(num_hidden)}}
        )
...
class LSTM(Symbolic):
    def __init__(self, num_hidden, act_type):
        ...
        hidden = mxnet.symbol.Variable("hidden")
        data = mxnet.symbol.FullyConnected(
            data=data, num_hidden=4 * num_hidden, no_bias=True
        )
    ...
    def forward(self, data, hidden=None, cell=None):
        ...
        if hidden is None:
            hidden = minpy.numpy.zeros((N, self._num_hidden))
        if cell is None:
            cell = minpy.numpy.zeros((N, self._num_hidden))
    ...
    def param_shapes(self, data_shape, hidden_shape=None, cell_shape=None):
        ...
        return super(LSTM, self).param_shapes(
            data=data_shape, hidden=hidden_shape, cell=cell_shape
        )
