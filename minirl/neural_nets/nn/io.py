...
import pickle  # pylint: disable=import-error, no-name-in-module
import numpy as np
...
class NDArrayIter(DataIter):
    ...
    def __init__(
        self, data, label=None, batch_size=1, shuffle=False, last_batch_handle="pad"
    ):
    ...
    def reset(self):
        if self.last_batch_handle == "roll_over" and self.cursor > self.num_data:
            self.cursor = (
                -self.batch_size + (self.cursor % self.num_data) % self.batch_size
            )
        else:
            self.cursor = -self.batch_size
    ...
    def _getdata(self, data_source):
        ...
        if self.cursor + self.batch_size <= self.num_data:
            if isinstance(data_source[0][1], array.Array):
                ...
                return [
                    array.wrap(x[1][self.cursor : self.cursor + self.batch_size])
                    for x in data_source
                ]
            else:
                raise TypeError(
                    "Invalid data type, only numpy.ndarray and minpy.array.Array are allowed."
                )
        else:
            ...
            if isinstance(data_source[0][1], array.Array):
                return [
                    array.wrap(
                        np.concatenate(
                            (x[1][self.cursor :].asnumpy(), x[1][:pad].asnumpy()),
                            axis=0,
                        )
                    )
                    for x in data_source
                ...
                return [
                    array.wrap(
                        np.concatenate((x[1][self.cursor :], x[1][:pad]), axis=0)
                    )
                    for x in data_source
                ]
            else:
