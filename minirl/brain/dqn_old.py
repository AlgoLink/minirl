...
class NN:  # neural_network
    def __init__(
        ...
        ob_n = input_shape
        H = hidden_neurons
        self.l1_weights = (-1 + 2 * np.random.rand(ob_n, H)) / np.sqrt(ob_n)
        ...
        self.l2_weights = (-1 + 2 * np.random.rand(H, ac_n)) / np.sqrt(H)
    ...
    def save_weights(self, model_id):
        if self._model_db is None:
            pickle.dump(
                [self.l1_weights, self.l1_biases, self.l2_weights, self.l2_biases],
