...
class Optimiser:
    def __init__(
        self, neural_net, learning_rate=1e-3, loss_fcn=None, act_loss_merged=True
    ):
...
class Adam(Optimiser):
    def __init__(
        ...
        self.v_weights = [
            np.zeros(w.shape) for w in neural_net.weights
        ]  # Stores weight momentum
        self.v_bias = [
            np.zeros(b.shape) for b in neural_net.biases
        ]  # Stores bias momentum
        self.m_weights = [
            np.zeros(w.shape) for w in neural_net.weights
        ]  # Stores weight momentum
        self.m_bias = [
            ...
            for batch_i in batch_ind:
                ...
                if self.loss_fcn is not None:
                    if self.loss_fcn is not loss.cross_entropy_loss:
                        ...
                        batch_target = target[batch_i]
                    batch_init_gradient = batch_init_gradient * self.loss_fcn_deriv(
                        batch_target, batch_estimate
                    )

                # Backpropagate errors and compute gradients
                (nabla_w, nabla_b) = self.neural_net.parameter_gradient(
                    init_gradient=batch_init_gradient,
                    act_loss_merged=self.act_loss_merged,
                )
                #                print(nabla_w)
                #                sys.exit(0)
                #                new_nabla_w = []
                ...
                self.m_weights = [
                    self.B1 * m + (1 - self.B1) * dw
                    for m, dw in zip(self.m_weights, nabla_w)
                ]
                self.m_bias = [
                    self.B1 * m + (1 - self.B1) * db
                    for m, db in zip(self.m_bias, nabla_b)
                ]

                self.v_weights = [
                    self.B2 * v + (1 - self.B2) * dw**2
                    for v, dw in zip(self.v_weights, nabla_w)
                ]
                self.v_bias = [
                    self.B2 * v + (1 - self.B2) * (db) ** 2
                    for v, db in zip(self.v_bias, nabla_b)
                ]

                # Compute learning rate
                alpha_t = (
                    learning_rate
                    * np.sqrt(1 - self.B2**self.t)
                    / (1 - self.B1**self.t)
                )

                # Update weights
                self.neural_net.biases = [
                    b - alpha_t * m / (np.sqrt(v) + self.eps)
                    for b, m, v in zip(self.neural_net.biases, self.m_bias, self.v_bias)
                ]
                self.neural_net.weights = [
                    w - alpha_t * m / (np.sqrt(v) + self.eps)
                    for w, m, v in zip(
                        self.neural_net.weights, self.m_weights, self.v_weights
                    )
                ]
...
class GradientDescent(Optimiser):
    def __init__(
        self, neural_net, learning_rate=1e-3, loss_fcn=None, variant="batch_GD"
    ):
        super().__init__(neural_net, learning_rate, loss_fcn)
    ...
    def train(self, x, target, num_epochs=1, init_gradient=None, learning_rate=None):
        self.optimiser(
            x, target, num_epochs=1, init_gradient=None, learning_rate=learning_rate
        )
    ...
    def batch_GD(self, x, target, num_epochs=1, init_gradient=None, learning_rate=None):
        ...
        for epoch in range(num_epochs):
            self.gradient_descent_update(
                x, target, init_gradient, learning_rate=learning_rate
            )
    ...
    def SGD(self, x, target, num_epochs=1, init_gradient=None, learning_rate=None):
        ...
        for epoch in range(num_epochs):
            ...
            for i in ind:
                ...
                self.gradient_descent_update(
                    x_i, target_i, batch_init_gradient, learning_rate=learning_rate
                )

    def batch_SGD(
        self, x, target, num_epochs=1, init_gradient=None, learning_rate=None
    ):
    ...
    def gradient_descent_update(
        self, batch_x, batch_target, init_gradient=None, learning_rate=None
    ):
        if learning_rate is None:
            learning_rate = self.learning_rate
        ...
        corrections = self.neural_net.parameter_gradient(
            init_gradient=batch_init_gradient, act_loss_merged=self.act_loss_merged
        )

        # Update weights
        (self.neural_net.weights, self.neural_net.biases) = update_weights(
            self.neural_net.get_weights(), corrections, learning_rate
        )
...
class Momentum(Optimiser):
    def __init__(
        ...
        self.gamma = 0.9
        self.v_weights = [
            np.zeros(w.shape) for w in self.neural_net.weights
        ]  # Stores weight momentum
        self.v_bias = [
            np.zeros(b.shape) for b in self.neural_net.biases
        ]  # Stores bias momentum
    ...
    def train(self, x, target, num_epochs=1, init_gradient=None, learning_rate=None):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_i in batch_ind:
                ...
                if self.nesterov:
                    weight_estimate, bias_estimate = update_weights(
                        self.neural_net.get_weights(),
                        (self.v_weights, self.v_bias),
                        learning_rate=self.gamma,
                    )
                else:
                    weight_estimate, bias_estimate = self.neural_net.get_weights()

                batch_estimate = self.neural_net.predict(
                    batch_x, weight_estimate, bias_estimate
                )
                ...
                # correct with momentum
                self.v_weights = [
                    self.gamma * v + learning_rate * dw
                    for v, dw in zip(self.v_weights, nabla_w)
                ]
                self.v_bias = [
                    self.gamma * v + learning_rate * db
                    for v, db in zip(self.v_bias, nabla_b)
                ]
