...
from .activation import get_act_fcn_deriv  # to get activation function derivatives
from . import init  # for default weight initialisation


class NeuralNetwork:
    def __init__(
        self, dims, act_fcns, model_db=None, init_fcn=init.glorot_uniform_init
    ):
        ...
        assert self.num_layers - 1 == len(
            act_fcns
        ), "Check number of activations is equal to number of layers -1"
        ...
        for act_fcn in self.act_fcn:
            self.act_fcn_deriv.append(get_act_fcn_deriv(act_fcn))
    ...
    def parameter_gradient(
        self, weights=None, biases=None, init_gradient=None, act_loss_merged=True
    ):
        ...
        nabla_w[-1] = (
            delta @ self.activations[-2].transpose() + self.reg * self.weights[-1]
        )
        ...
        for l in range(2, self.num_layers):
            ...
            nabla_w[-l] = (
                delta @ self.activations[-l - 1].transpose()
                + self.reg * self.weights[-l]
            )
    ...
    def input_gradient(self, x):
        ...
        for l in range(2, self.num_layers):
            z = self.zs[-l]
            delta = (self.weights[-l + 1].transpose() @ delta) * self.act_fcn_deriv[-l](
                z
            )
    ...
    def get_weights(self, name):
        ...
        weights, biases = self.load_weights(name)
    ...
    def blend_weights(self, target_network, tau=0.1):
        self.weights = [
            tau * target_weights + (1 - tau) * weights
            for weights, target_weights in zip(self.weights, target_network.weights)
        ]
        self.biases = [
            tau * target_biases + (1 - tau) * biases
            for biases, target_biases in zip(self.biases, target_network.biases)
        ]

    def save_weights(self, name):
        if self._model_db is None:
            pickle.dump(
                [self.weights, self.biases], open("{}.pickle".format(name), "wb")
            )
        else:
            model_key = f"{name}:wgt"
            self._model_db.set(model_key, pickle.dumps([self.weights, self.biases]))

    def load_weights(self, name):
        try:
            if self._model_db is None:
                weights, biases = pickle.load(open("{}.pickle".format(name), "rb"))
            else:
                model_key = f"{name}:wgt"
