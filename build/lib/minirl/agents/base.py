...
class Actor:
    def __init__(
        ...
        self.target_model.set_weights(*self.model.get_weights(wgt_name))
        self.optimiser = optim.Adam(
            self.model,
            learning_rate,
            loss_fcn=loss.cross_entropy_loss,
            batch_size=batch_size,
        )

    def create_model(self, num_states, num_actions, hidden_layers):
        model = NeuralNetwork(
            [num_states, *hidden_layers, num_actions],
    ...
    def train(self, states, actions, action_gradient):
        num_samples = states.shape[1]
        self.optimiser.train(
            states,
            np.array(actions),
            # np.zeros((self.num_actions, num_samples)),
            init_gradient=-self.env_action_max * action_gradient,
        )
...
class Critic:
    def __init__(
        ...
        self.optimiser = optim.Adam(
            self.model,
            learning_rate,
            loss_fcn=loss.mean_squared_error,
            batch_size=batch_size,
        )
    ...
    def train(self, state, target):
        ...
        self.optimiser.train(state, target)
