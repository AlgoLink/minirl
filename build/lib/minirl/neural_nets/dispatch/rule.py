...
class Rules(object):
    ...
    def __init__(self, loc=None, save_config_atexit=False):
        ...
        if save_config_atexit and not self._save_config_atexit:
            self.__class__._save_config_atexit = (
        ...
        if cls._rules is None or force:
            ...
            for filename in locs:
                try:
                    ...
                    _logger.warning("Find corrupted configuration at %s", filename)
            if config is None:
                _logger.error(
                ...
                _logger.info(
                    "Load and use rule configuration at %s", filename
...
class Blacklist(Rules):
    ...
    def allow(self, name, nspace, impl_type, args, kwargs):
        ...
        if (
            nspace in self._hash
            and name in self._hash[nspace]
            and (
                self._hash[nspace][name] is None
                or self._get_arg_rule_key(args, kwargs) in self._hash[nspace][name]
            )
        ):
            _logger.debug("Rule applies: block by auto-generated rule on %s.", name)
            return False
    ...
    def add(self, name, nspace, impl_type, args, kwargs):
        ...
        if self._get_arg_rule_key(args, kwargs) not in self._hash[nspace][name]:
    ...
    def _build_hash(cls):
        ...
        for nspace, ns_rules in cls._rules.items():
            ...
            for key, rules in ns_rules.items():
                ...
                for elm in rules:
                    cls._hash[nspace][key].add(
                        "-".join(elm["args"])
                        + "+"
                        + "-".join(sorted(elm.get("kwargs", [])))
                    )
    ...
    def query(cls, nspace, name):
        ...
        if ns_path not in cls._rules or name not in cls._rules[ns_path]:
            ...
            return (
                "Total: {} blacklist rules for primitive [{}]:\n".format(
                    len(rule_list), name
                )
                + table
            )
