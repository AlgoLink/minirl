...
class PrimitivePolicyError(ValueError):
    ...
    def __init__(self, name, policy_name):
        super(PrimitivePolicyError, self).__init__(
            "Cannot find implementation for function: {}() "
...
class Policy(object):
    ...
    def show_op_stat(self):
        ...
        if total_cnt > 0:
            print(
                "Total Dispatch Proportion: {:.1%} in MXNet, {:.1%} in NumPy".format(
                    float(mxnet_op_cnt) / total_cnt, float(numpy_op_cnt) / total_cnt
                )
            )
...
class AutoBlacklistPolicy(Policy):
    ...
    def resolve_call(self, name, reg, args, kwargs):
        ...
        if ArrayType.MXNET in possible_impl and self._rules.allow(
            ...
            if self._gen_rule:
                try:
                    _logger.debug("Try primitive %s with MXNet implementation.", name)
                ...
                _logger.debug("Execute primitive %s with MXNet implementation", name)
                self._mxnet_op_stat[name] += 1
                return _get_result(ArrayType.MXNET)
        elif ArrayType.NUMPY in possible_impl:
            _logger.debug("Execute primitive %s with NumPy implementation", name)
            self._numpy_op_stat[name] += 1
            return _get_result(ArrayType.NUMPY)
        else:
            raise PrimitivePolicyError(name, self.name)
...
def wrap_policy(plc):
    ...
    def policy_decorator(func):
        ...
        def policy_wrapper(*args, **kwargs):
            old_policy = minirl.neural_nets.Config["default_policy"]
            minirl.neural_nets.set_global_policy(plc)
            result = func(*args, **kwargs)
            minirl.neural_nets.set_global_policy(old_policy)
            return result
