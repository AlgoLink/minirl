...
class Primitive(object):
    ...
    def _convert_data(self, data, mutate):
        ...
        if (
            ...
            and len(data) != 0
            and isinstance(data[0], array.Value)
        ):
        ...
        if need_bp:
            ...
            for i in bp_idx:
                ...
                if i not in self._grad_func:
                    _logger.debug(
                        ...
                        "argument {} is not defined.".format(
                    ...
                    _logger.debug(
                        'Adding partial derivative to func "{}" on argument {}.'.format(
                            self._func, i
                        )
                    )
                    ...
                    # for later use.
                    grad_func = grad_func_rec.f(
                        result_value, *arg_values, **kwarg_values
                    )
                    ...
                    if grad_func_rec.multi_grad_indices is None:
                        ...
                        for grad_index in grad_func_rec.multi_grad_indices:
                            if isinstance(args[grad_index], array.Value) and args[
                                grad_index
                            ].is_marked_for_bp(current_tape):
                ...
                if isinstance(result, tuple):
                    for elm in result:
                        current_tape.add_partial_derivative(grad_func, owner, elm)
                else:
            ...
            for k in bp_kw:
                ...
                if k not in self._grad_func_kw:
                    ...
                    _logger.debug(
                        'Adding partial derivative to func "{}" on keyword argument "{}".'.format(
                            self._func, k
                        )
                    )
    ...
    def def_multiple_grad(self, func, argnums):
        ...
        for argnum in argnums:
            assert isinstance(argnum, int), "Indexes must be tuple of integers."
            self._grad_func[argnum] = GradFunc(f=func, multi_grad_indices=argnums)
        return self
    ...
    def gradable(self, bp_args, bp_kwargs):
        ...
        """
        return all(i in self._grad_func for i in bp_args) and all(
            i in self._grad_func_kw for i in bp_kwargs
        )
