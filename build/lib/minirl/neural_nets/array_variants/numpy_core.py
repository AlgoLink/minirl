...
def _match_shape(a, axis, keepdims):
    ...
    """
    assert isinstance(
        axis, (type(None), int, tuple)
    ), "axis must be None, int, or tuple."
    ...
    if axis is None:
        ...
        if keepdims:
            ...
            return (
                lambda g: np.repeat(np.expand_dims(g, axis), shape[axis], axis),
                shape[axis],
            )
    else:
...
def register_primitives(reg, prim_wrapper):
    ...
    wrap_namespace(np.__dict__, reg, prim_wrapper)
...
def def_grads(prims):
    ...
    # Binary functions
    prims("multiply").def_grad(lambda ans, x, y: _unbroadcast(ans, x, lambda g: g * y))
    ...
    prims("add").def_grad(lambda ans, x, y: _unbroadcast(ans, y, _identity), argnum=1)
    ...
    prims("divide").def_grad(lambda ans, x, y: _unbroadcast(ans, x, lambda g: g / y))
    prims("divide").def_grad(
        lambda ans, x, y: _unbroadcast(ans, y, lambda g: -g * x / y**2), argnum=1
    )
    ...
    prims("mod").def_grad(
        lambda ans, x, y: _unbroadcast(ans, y, lambda g: -g * np.floor(x / y)), argnum=1
    )
    ...
    prims("power").def_grad(
        lambda ans, x, y: _unbroadcast(ans, y, lambda g: g * np.log(x) * ans), argnum=1
    )
    prims("maximum").def_grad(
        lambda ans, x, y: _unbroadcast(ans, x, lambda g: g * (x == ans))
    )
    prims("maximum").def_grad(
        lambda ans, x, y: _unbroadcast(ans, y, lambda g: g * (y == ans)), argnum=1
    )
    ...
    prims("reshape").def_grad(lambda ans, x, _1: lambda g: np.reshape(g, x.shape))
    prims("append").def_grad(
        lambda ans, arr, values, axis=None: lambda g: np.split(
            g, [arr.shape[axis]], axis
        )[0]
    )
    prims("append").def_grad(
        lambda ans, arr, values, axis=None: lambda g: np.split(
    ...
    prims("expand_dims").def_grad(lambda ans, x, axis: lambda g: np.reshape(g, x.shape))
