...
from minirl.neural_nets import array
from minirl.neural_nets import tape
...
def _is_array(array):
    return isinstance(array, (array.Array, numpy.ndarray))


def _is_generic_minpy_array(array):
    return isinstance(array, (array.Array, array.Number))
...
def _register_configs(configs, to_register):
    ...
    for identifier, attr_value in to_register.items():
        assert isinstance(
            identifier, str
        ), KeyError()  # global attribute must be str (e.g. 'learning_rate')
...
class Layer(Module):
    def __init__(self, params=None, aux_params=None, name=None):
        ...
        self._param_names = self._assign_param_names(
            *params
        )  # global param names (identifiable in model)
        ...
        self._aux_param_names = self._assign_param_names(
            *aux_params
        )  # global aux param names (identifiable in model)
        ...
        default_init_configs = {
            name: self._get_default_init_config(name)
            for name in self._module_param_names
        }
        default_init_configs.update(
            {
                name: self._get_default_init_config(name)
                for name in self._module_aux_param_names
            }
        )
    ...
    def _init_params(self, param_shapes):
        ...
        for name, shape in param_shapes.items():
            ...
            if name not in self._model.params:
                ...
                self._model.params[name] = getattr(init, init_config["init_rule"])(
                    shape, init_config
                )
    ...
    def _init_aux_params(self, aux_param_shapes):
        ...
        for name, shape in aux_param_shapes.items():
            ...
            if name not in self._model.aux_params:
                init_config = self._init_configs[name]
                self._model.aux_params[name] = getattr(init, init_config["init_rule"])(
                    shape, init_config
                )
    ...
    def _get_aux_params(self, *aux_param_names):
        # should not be called prior to calling __call__ for the first time
        return tuple(
            self._model.aux_params[aux_param_name] for aux_param_name in aux_param_names
        )
    ...
    def _parse_param_configs(self, configs):
        ...
        for identifier in _configs:
            if isinstance(
                identifier, tuple
            ):  # configs applied to a group of parameters
                config = _configs.pop(identifier)
                # expand group configs to parameter-specific configs
                for param_name in identifier:
                    _configs[param_name] = config

        # convert local (module) name to global name
        for module_param_name, param_name in zip(
            self._module_param_names, self._param_names
        ):
            if module_param_name in _configs:
                _configs[param_name] = _configs.pop(module_param_name)

        # convert local (module) name to global name
        for module_aux_param_name, aux_param_name in zip(
            self._module_aux_param_names, self._aux_param_names
        ):
    ...
    def aux_param_dict(self):
        return dict(
            zip(self._aux_param_names, self._get_aux_params(*self._aux_param_names))
        )
...
class Model(model.ModelBase):
    ...
    def __call__(
        ...
        tape.Tape.global_tape = self._tape
        ...
        if forward_args is None:
            forward_args = tuple()
        elif not isinstance(forward_args, tuple):
            forward_args = (forward_args,)
        forward_args = tuple(map(array.wrap, forward_args))
        ...
        assert _is_generic_minpy_array(predictions) and (
            ...
            if isinstance(predictions, collections.Iterable)
            else True
        ...
        if loss is None:
            ...
            if callable(loss):
                ...
                if not isinstance(labels, tuple):
                    labels = (labels,)
                labels = tuple(map(array.wrap, labels))
    ...
    def backward(self, upstream=None):
        wrapped_sources = map(array.wrap, self._bp_array_list)
        if isinstance(self._results, tuple):
            wrapped_targets = map(array.wrap, self._results)
        else:
            wrapped_targets = array.wrap(self._results)
...
class _ConfigParser(object):
    ...
    def __getattr__(self, attr):
        attr_values = set(
            config[attr] for config in self._configs.values() if attr in config
        )
...
class Updater(_ConfigParser):
    ...
    def __call__(self, grad_dict):
        ...
        for param_name, grad in grad_dict.items():
            ...
            self._model.params[param_name], _update_config = getattr(
                optim, update_rule
            )(param, grad, update_config)
