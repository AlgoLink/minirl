...
GradRecord = collections.namedtuple("GradRecord", ["grad_func", "result", "owner"])
...
class Tape(object):
    ...
    def _set_gradient_target(self, target):
        ...
        if isinstance(target, array.Value):
            self._grads[target.id] = array.wrap(
                1.0 if isinstance(target, array.Number) else numpy.ones(target.shape)
            )
        else:
    ...
    def get_gradient(self, origin, target):
        ...
        while len(bfs_queue) != 0:
            ...
            while len(grad_records) != 0:
                ...
                _logger.debug('Calling derivative func "%s"', grad_record.grad_func)
                ...
                if isinstance(owner, array.Value):
                    ...
                    for sub_owner in owner:
                        if isinstance(sub_owner, array.Value) and decr_refcount(
                            sub_owner
                        ):
