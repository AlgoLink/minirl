...
from minirl.neural_nets.array_variants import variants
from minirl.neural_nets.dispatch.registry import Registry
from minirl.neural_nets.dispatch.primitive_selector import PrimitiveSelector
from minirl.neural_nets.primitive import Primitive
...
class Module(object):
    ...
    It will register primitives from :mod:`minirl.neural_nets.array_variant`.
    ...
    def __init__(self, old, name=None, name_injector=None):
        ...
        minirl.Config["modules"].append(self)
        ...
        for vname in variants:
            ...
            if name is None:
                modname = "minirl.neural_nets.array_variants.{}".format(vname)
            else:
                modname = "minirl.neural_nets.array_variants.{}.{}".format(vname, name)
            mod = importlib.import_module(modname)
            self._logger.info("Importing from %s.", modname)
            primitive_wrapper = lambda func, *args, **kwargs: Primitive(
                func, vtype, *args, **kwargs
            )
            ...
            self._logger.info(
                "Got %d primitives from %s", len(self._registry._reg) - before, modname
            )
        ...
        self.generate_attrs(minirl.Config["default_policy"])
    ...
    def generate_attrs(self, policy, use_selector=False):
        ...
        for (
            k,
            prims,
        ) in self._registry._reg.items():  # pylint: disable= protected-access
            if not use_selector:
...
class NameInjector(object):
    ...
    def __init__(
        self, dest_mod, name="", injected_type=None, name_set=None, exception=None
    ):
        ...
        if exception is not None:
            ...
            self._logger.debug(
                "Update %d exceptions to Name Injector %s",
                len(exception),
                self.__name__,
            )
            self._name_dict.update(exception)
        self._logger.info(
            "Import %d objects from %s to Name Injector %s",
            len(self),
            dest_mod.__name__,
            self.__name__,
        )
