...
class Smab:
    ...
    def __init__(
        self, action_ids, success_priors=None, failure_priors=None, random_seed=None
    ):
        """Initialization."""
        if random_seed is not None:
            random.seed(random_seed)
        self._actions_ids = action_ids
        self._success_counters = (
            dict((t, 1) for t in self._actions_ids)
            if success_priors is None
            else copy.deepcopy(success_priors)
        )
        """ dict: success counters initialized to 1 for each action"""
        self._failure_counters = (
            dict((t, 1) for t in self._actions_ids)
            if failure_priors is None
            else copy.deepcopy(failure_priors)
        )
        """dict: failure counters initialized to 1 for each action"""

        # Input sanity checking
        if (success_priors is None and failure_priors is not None) or (
            success_priors is not None and failure_priors is None
        ):
            raise ValueError(
                "Either both or neither success_prior and failure_prior should be specified."
            )
        if set(self._success_counters.keys()) != set(action_ids) or set(
            self._failure_counters.keys()
        ) != set(action_ids):
            raise ValueError(
                "Treatment IDs, successes keys and failure keys must be identical."
            )
        if len(action_ids) < 1:
        ...
        if (
            type(self._success_counters) is not dict
            or type(self._failure_counters) is not dict
        ):
        ...
        for t in self._actions_ids:
            ...
            if (
                type(self._success_counters[t]) is not int
                or type(self._failure_counters[t]) is not int
            ):
    ...
    def predict(self, n_samples=1, forbidden_actions=None):
        ...
        for _ in range(n_samples):
            p = {
                t: random.betavariate(
                    self._success_counters[t], self._failure_counters[t]
                )
                for t in self._actions_ids
    ...
    def update(self, action_id, n_successes, n_failures):
        ...
        if (
            ...
            or type(n_successes) is not int
            or type(n_failures) is not int
        ):
            raise TypeError(
                "action_id must be a string and n_successes/failures must be integers."
            )
    ...
    def batch_update(self, batch):
        ...
        for elem in batch:
            ...
            assert (
                "action_id" in elem and "n_successes" in elem and "n_failures" in elem
            ), "Batch must be in the form [{'action_id': <str>, 'n_successes': <int>, 'n_failures':<int>}]"
            self.update(
                action_id=elem["action_id"],
                n_successes=elem["n_successes"],
                n_failures=elem["n_failures"],
            )
