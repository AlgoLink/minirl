    ...
    def learn(
        ...
        if recom_his is not None:
            ...
            if str(_request_id) == str(request_id):
                ...
                Q1_state_value = self.get_Q_value(
                    q_model, _next_state, argmax_Q1_action
                )
                Q0_state_value = self.get_Q_value(q_model, _state, str(_action))

                td_error = reward + self.gamma * Q1_state_value - Q0_state_value
                Q0_state_value += self.alpha * td_error

                q_model[(_state, str(_action))] = Q0_state_value
                self.update_q_model(q_model, model_id)
        ...
        if len(dyna_model.model.keys()) > planning_steps and planning_steps > 0:
            ...
            for t in range(0, planning_steps):
                ...
                Q1_state_value = self.get_Q_value(
                    q_model, next_state_, argmax_Q1_action
                )
                Q0_state_value = self.get_Q_value(q_model, state_, action_)

                td_error = reward_ + self.gamma * Q1_state_value - Q0_state_value
                Q0_state_value += self.alpha * td_error

                q_model[(state_, action_)] = Q0_state_value
