...
class Cmab:
    ...
    def __init__(
        ...
        if type(actions_ids) is not list or not all(
            isinstance(x, str) for x in actions_ids
        ):
        ...
        self._models = (
            {}
        )  # dictionary of pymc3 models per each action: keys=actions_ids, values=pymc3.Model()
    ...
    def _init_alpha(self, prior, default):
        ...
        if (
            ...
            raise TypeError(
                "prior must be a dict with string as keys and float as values. prior =",
                prior,
            )
        if set(prior.keys()) != set(self._actions_ids):
            raise ValueError(
                "prior dict keys must be equal to the actions_ids. prior =", prior
            )
        return prior
    ...
    def _init_betas(self, prior, default):
        ...
        if prior is None:
            prior = {
                ...
                for (k, v) in zip(
                    self._actions_ids,
                    len(self._actions_ids) * [self._n_features * [default]],
                )
            }
        if (
            type(prior) is not dict
            ...
            raise TypeError(
                "prior must be a dict with string as keys and lists as values. prior =",
                prior,
            )
        if not all(
            [
                item
                for list in [[isinstance(x, float) for x in v] for v in prior.values()]
                for item in list
            ]
        ):
        ...
        if not np.all(
            np.array([len(x) for x in list(prior.values())]) == self._n_features
        ):
            raise ValueError(
                "prior values must be lists of length = n_features. prior =", prior
            )
        return prior
    ...
    def _update_trace(self, alpha, betas, X, rewards):
        ...
        linear_combination = Deterministic(
            "linear_combination", alpha + dot(betas, X.T)
        )
    ...
    def _worker_update(self, X, actions, rewards, a):
        ...
        with Model() as model:
            ...
            alpha = StudentT(
                "alpha",
                mu=self._mu_alpha[a],
                sigma=self._sigma_alpha[a],
                nu=self._nu_alpha[a],
            )
            betas = [
                StudentT(
                    "beta" + str(j),
                    mu=self._mu_betas[a][j],
                    sigma=self._sigma_betas[a][j],
                    nu=self._nu_betas[a][j],
                )
                for j in range(len(X.columns))
            ]

            # update traces
            trace = self._update_trace(
                alpha, betas, X[actions == a], rewards[actions == a]
            )
    ...
    def update(self, X, actions, rewards):
        ...
        try:
            ...
            print(
                "{}\nCannot convert input arguments to pandas DataFrame/Series".format(
                    e
                )
            )
            raise
        ...
        if self._n_jobs == 1:
            new_models = [
                self._worker_update(X, actions, rewards, a) for a in set(actions)
            ]
        else:
            ...
            # update pymc3 models
            new_models = p.map(
                partial(self._worker_update, X, actions, rewards), set(actions)
            )
        ...
        for a in set(actions):
            ...
            self._traces[a], self._models[a] = next(
                (
                    (trace, model)
                    for (action, trace, model) in new_models
                    if action == a
                ),
                None,
            )
            ...
            self._mu_betas[a] = [
                np.mean(self._traces[a]["beta" + str(j)]) for j in range(len(X.columns))
            ]
            self._sigma_alpha[a] = np.std(self._traces[a]["alpha"], ddof=1)
            self._sigma_betas[a] = [
                np.std(self._traces[a]["beta" + str(j)], ddof=1)
                for j in range(len(X.columns))
            ]
    ...
    def _worker_predict(self, X, a):
        ...
        with self._models[a]:
            ...
            pps = fast_sample_posterior_predictive(
                trace=self._traces[a],
                random_seed=self._random_seed,
                var_names=["linear_combination"],
                samples=1,
            )
    ...
    def _predict_random_actions(self, X):
        ...
        return rng.choice(self._actions_ids, size=len(X)), np.full(
            (len(self._actions_ids), len(X)), 0.5
        )
